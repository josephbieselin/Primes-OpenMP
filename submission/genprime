 ...
 */
void generate_primes(int N, int t, int *nums)
{
	int i, j;
...
int main(int argc, char *argv[])
{
	...
	int *nums = (int *) malloc((N+1) * sizeof(int)); // allocate array to store 2, ..., N
	// store the numbers from 2, ..., N in the associated array position: 2->2, ..., N->N
	for (i = 2; i <= N; ++i) nums[i] = i;

	generate_primes(N, t, nums);
